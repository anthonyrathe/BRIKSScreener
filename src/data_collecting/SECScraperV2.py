...
class SECScraperV2:
	...
	def scrape(self):
		sic = "0000320193"
		filing = Filing(sic,FilingType.FILING_10Q,count=1)
		path = os.path.relpath("{}/data/raw/working_dir/".format(dirname(dirname(dirname(__file__)))))
		data = filing.scrape(path)[0]

		data = data[data.find("</us-gaap:RevenueRemainingPerformanceObligationExpectedTimingOfSatisfactionPeriod1>\n    <dei:DocumentType\n      contextRef="):]
		data = data[:1000]
